...
import { getMarket, getDefaultMarket, getAllMarkets } from './markets.js';
dotenv.config();
const OPERATOR_PRIVATE_KEY = process.env.OPERATOR_PRIVATE_KEY;
const PERPETUAL_ADDRESS = process.env.PERPETUAL_CONTRACT_ADDRESS; // Legacy support
 ...
 */
export const getPerpetualContract = (marketSymbol) => {
    let contractAddress;
    if (marketSymbol) {
 ...
 */
export const getMarketConfig = (marketSymbol) => {
    if (marketSymbol) {
        const market = getMarket(marketSymbol);
        if (!market) {
